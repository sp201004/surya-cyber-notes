...
with open('/Users/sp2010/.gemini/antigravity-ide/brain/8856246e-6dd3-4c6a-bb79-23c82855cc26/scratch/course1.json', 'r') as f:
    modules = json.load(f)

mod1 = modules[0]
mod1['description'] = "Core foundational concepts of cybersecurity including the CIA triad, threats, and frameworks."

# Populate Room 1
mod1['topics'][0]['realWorldCallout'] = {
    "title": "Case Study",
...
# Populate Room 2
mod1['topics'][1]['realWorldCallout'] = {
    "title": "Case Study",
...
# Populate Room 3
mod1['topics'][2]['realWorldCallout'] = {
    "title": "Case Study",
...
# Populate Room 4
mod1['topics'][3]['realWorldCallout'] = {
    "title": "Case Study",
...
mod1['topics'][3]['quiz'] = [
    {"id": "g-c1-r4-q1", "question": "What are the three incident management priorities in order?", "type": "text", "correctAnswer": "Containment, Root Cause Investigation, Execute Playbook.", "hint": "Stop the bleeding first."},
    {"id": "g-c1-r4-q2", "question": "How does an IDS anomaly detection differ from signature detection?", "type": "text", "correctAnswer": "Anomaly detects deviations from baselines (catching zero-days); signature matches known attack patterns.", "hint": "Unknown deviations vs known patterns."},
    {"id": "g-c1-r4-q3", "question": "What is an APT?", "type": "text", "correctAnswer": "Advanced Persistent Threat; a nation-state group that stays undetected for long periods.", "hint": "Nation-state level actors."},
    {"id": "g-c1-r4-q4", "question": "What is the purpose of the Principle of Least Privilege?", "type": "text", "correctAnswer": "To grant users only the minimum access required, limiting the potential damage of a compromise.", "hint": "Only giving people the keys they absolutely need."}
]

# Add mindmap property to all topics
